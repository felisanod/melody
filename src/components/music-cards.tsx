...
export function ItemCard({ item, contextSongs }: { item: MusicItem; contextSongs?: SongItem[] }) {
  ...
  if (item.kind === "artist")
    return (
      <Link to="/artist/$artistId" params={{ artistId: item.id }} className="group w-40 shrink-0 sm:w-44">
        {shared}
      </Link>
    );
  return (
...
export function ShelfRow({ shelf }: { shelf: Shelf }) {
  ...
  return (
    <section className="space-y-3">
      <h2 className="font-display text-xl font-semibold tracking-tight sm:text-2xl">{shelf.title}</h2>
...
export function SongRow({
  ...
  return (
    <div
      className={cn(
        "group flex items-center gap-3 rounded-xl px-2 py-2 transition-colors hover:bg-secondary/70",
        active && "bg-secondary",
      )}
    >
      {position !== undefined && (
      ...
      <button
        ...
        title="Add to queue"
        className="rounded-full p-2 text-muted-foreground opacity-0 transition hover:bg-secondary hover:text-foreground group-hover:opacity-100"
      >
...
export function EmptyState({ icon = "list", message }: { icon?: "list" | "user"; message: string }) {
  const Icon = icon === "user" ? User : ListMusic;
  return (
    <div className="flex flex-col items-center gap-3 rounded-2xl border border-border/60 bg-card/40 px-6 py-16 text-center">
